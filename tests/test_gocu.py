...
import takip_botu_pro as bot


def _kur(tmp_path, monkeypatch):
    """Geçici dosyalarla göç ortamı hazırlar."""
    monkeypatch.setattr(bot, "HISTORY_CSV", tmp_path / "gecmis.csv")
    monkeypatch.setattr(bot, "TELEGRAM_URUNLER", tmp_path / "tg.yaml")

    with open(bot.HISTORY_CSV, "w", newline="", encoding="utf-8") as f:
    ...
    bot.save_yaml_atomic(bot.TELEGRAM_URUNLER, {
        "eklenen": [], "kaldirilan": [],
        "hedefler": {"Eski Ad": 90.0},
        "ek_kaynaklar": {"Eski Ad": ["http://akakce/x"]},
    })

    state = bot.State(tmp_path / "state.json")
    state.data = {
...
def test_goc_tasima(tmp_path, monkeypatch):
    ...
    assert bot.etiket_gocu(state, products) is True
    ...
    rows = list(csv.reader(open(bot.HISTORY_CSV, encoding="utf-8"), delimiter=";"))
    assert rows[1][1] == "Yeni Ad" and rows[2][1] == "Baska Urun"

    # Telegram overlay anahtarları taşındı
    tg = bot.load_yaml(bot.TELEGRAM_URUNLER)
...
def test_goc_ikinci_calistirma_noop(tmp_path, monkeypatch):
    state, products = _kur(tmp_path, monkeypatch)
    bot.etiket_gocu(state, products)
    assert bot.etiket_gocu(state, products) is False
